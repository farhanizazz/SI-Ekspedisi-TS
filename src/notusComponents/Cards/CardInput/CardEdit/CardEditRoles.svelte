<script>
  ...
  onMount(async () => {
    const res = await fetch(`${mainUrl}/api/master/roles/${id}`, {
      headers: {
  ...
  async function handleSubmit() {
    fetch(`${mainUrl}/api/master/roles/${id}`, {
      headers: {
  ...
  $: Object.keys(data.akses).forEach((key) => {
    if (!data.akses[key].view) {
      data.akses[key].create = false;
      data.akses[key].edit = false;
      data.akses[key].delete = false;
    }
  });
</script>
