<script lang="ts">
	import { currentPage } from '../Invoice/stores/InvoiceStore.js';
  import { currentPageDetail } from "./stores/laporanHutangSopirStore.js";
  ...
  import type { LaporanHutangSopirId, LaporanHutangSopirIdRepository, LaporanHutangSopirRepository } from "src/data/repository/laporanHutangSopirRepository.js";
  ...
  export let onLoad = () => {};
  export let repository: LaporanHutangSopirIdRepository;
  export let id;
...
<div style="display: flex; justify-content: center; align-items: center;">
  <Pagination
    onLast={() => {
      repository.updatePage($currentPageDetail + 1);
    }}
    onFirst={() => {
      repository.updatePage($currentPageDetail + 1);
    }}
    ...
    {pageCount}
    onNext={() => {
      currentPageDetail.set($currentPageDetail + 1);
      repository.updatePage($currentPageDetail + 1);
    }}
    onPrev={() => {
      currentPageDetail.set($currentPageDetail - 1);
    ...
    onSeek={(page) => {
      currentPageDetail.set(page);
      console.log("Page: ", $currentPageDetail);
      repository.updatePage($currentPageDetail + 1);
    }}
  />
</div>
        ...
        <tbody>
          {#each data as tableData, index}
            ...
            <tr>
              ...
              >
                {tableData.tgl_transaksi}
              </td>
              <td
                class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-2 border"
              >
                {IDRFormatter.format(tableData.nominal_trans)}
              </td>
              <td
                class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-2 border"
              >
